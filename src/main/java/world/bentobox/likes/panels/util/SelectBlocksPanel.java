...
public class SelectBlocksPanel
{
    ...
    private PanelItem createMaterialButton(Material material)
    {
        ...
        String name = this.user.getTranslation(Constants.BUTTONS + "material-icon.name",
            Constants.PARAMETER_BLOCK, Utils.prettifyObject(this.user, material));
        ...
        return new PanelItemBuilder().
            ...
            clickHandler((panel, user1, clickType, slot) -> {
                if (this.singleSelect)
                {
                    this.consumer.accept(true, Collections.singleton(material));
                }
                else
                {
                ...
                if (!this.selectedMaterials.isEmpty())
                {
                    ...
                    for (Material material : this.selectedMaterials)
                    {
                        description.add(this.user.getTranslation(reference + "list-value",
                            Constants.PARAMETER_VALUE, Utils.prettifyObject(this.user, material)));
                    }
                }
...
// ---------------------------------------------------------------------
    ...
    {
        new SelectBlocksPanel(user, true, false, new HashSet<>(), consumer).build();
    }
