...
/**
 * A Red-Black tree based {@link java.util.NavigableMap} implementation. The map is sorted according to the {@linkplain
 * Comparable natural ordering} of its keys, or by a {@link java.util.Comparator} provided at map creation time,
 ...
 * explicit comparator is provided) must be <i>consistent with equals</i> if this sorted map is to correctly implement
 * the <tt>Map</tt> interface.  (See
 ...
 * <tt>compareTo</tt> (or <tt>compare</tt>) method, so two keys that are deemed equal by this method are, from the
 ...
 * mappings; merely changing the value associated with an existing key is not a structural modification.)  This is
 * typically accomplished by synchronizing on some object that naturally encapsulates the map. If no such object exists,
 * the map should be "wrapped" using the {@link java.util.Collections#synchronizedSortedMap
 * Collections.synchronizedSortedMap} method.  This is best done at creation time, to prevent accidental
 ...
 * java.util.ConcurrentModificationException}.  Thus, in the face of concurrent modification, the iterator fails quickly
 ...
 * modification.  Fail-fast iterators throw <tt>ConcurrentModificationException</tt> on a best-effort basis. Therefore,
 ...
 * and its views represent snapshots of mappings at the time they were produced. They do <em>not</em> support the
 * <tt>Entry.setValue</tt> method. (Note however that it is possible to change mappings in the associated map using
 * <tt>put</tt>.)
 * <p/>
    ...
    implements IndexedNavigableMap<K, V>, Cloneable, java.io.Serializable
{
     ...
     * implement the {@link Comparable} interface.  Furthermore, all such keys must be
     ...
     * map that violates this constraint (for example, the user attempts to put a string key into a map whose keys are
     * integers), the
     ...
     * <tt>ClassCastException</tt> for any keys
     * <tt>k1</tt> and <tt>k2</tt> in the map.  If the user attempts to put
     * a key into the map that violates this constraint, the <tt>put(Object key, Object value)</tt> call will throw a
     * <tt>ClassCastException</tt>.
     *
     * @param comparator the comparator that will be used to order this map. If <tt>null</tt>, the {@linkplain
     ...
     * {@code k} to a value {@code v} such that {@code key} compares equal to {@code k} according to the map's ordering,
     ...
     * {@code null}. The {@link #containsKey containsKey} operation may be used to distinguish these two cases.
     *
     * @throws ClassCastException if the specified key cannot be compared with the keys currently in the map
     * @throws NullPointerException if the specified key is null and this map uses natural ordering, or its comparator
     * does not permit null keys
    ...
    /**
     * Returns a shallow copy of this <tt>IndexedTreeMap</tt> instance. (The keys and values themselves are not
     * cloned.)
     *
     ...
     * ascending order. The set is backed by the map, so changes to the map are reflected in the set, and vice-versa.
     ...
     * are reflected in the collection, and vice-versa.  If the map is modified while an iteration over the collection
     * is in progress (except through the iterator's own <tt>remove</tt> operation), the results of the iteration are
     * undefined.  The collection supports element removal, which removes the corresponding mapping from the map, via
     ...
     * own <tt>remove</tt> operation, or through the
    ...
    /**
     * Returns the first Entry in the IndexedTreeMap (according to the IndexedTreeMap's key-sort function).  Returns
     * null if the IndexedTreeMap is empty.
     */
     ...
     * by the key (Object) and value (Object) for each key-value mapping represented by the IndexedTreeMap. The
     * key-value mappings are emitted in key-order (as determined by the IndexedTreeMap's Comparator, or by the keys'
     * natural ordering if the IndexedTreeMap has no Comparator).
     */
    ...
    /**
     * This class exists solely for the sake of serialization compatibility with previous releases of IndexedTreeMap
     * that did not support NavigableMap.  It translates an old-version SubMap into a new-version AscendingSubMap. This
